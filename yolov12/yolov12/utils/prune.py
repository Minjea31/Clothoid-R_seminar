...
# 
#     @classmethod
#     def apply(cls, module, name, *args, importance_scores=None, module_name=None, **kwargs):
...
class BasePruningMethod(ABC):
    ...
    @classmethod
    def apply(cls, module, name, module_name, *args, importance_scores=None, **kwargs):
        ...
        def _get_composite_method(cls, module, name, module_name=module_name, *args, **kwargs):
            # Check if a pruning method has already been applied to
            # `module[name]`. If so, store that in `old_method`.
            old_method = None
            found = 0
            ...
            if old_method is not None:  # meaning that there was a hook
                ...
                # container, add the old pruning method and the new one
                    ...
                    method = container  # rename container --> method
            return method
            # 이 mothod가 뭘 의미하느지 확인 


        # =========================
        # 예외 분기만 추가 (name is None)
        # =========================
        if name is None:
            # 텐서 직접 프루닝 경로: module 인자에 torch.Tensor가 들어온 것으로 간주
            assert isinstance(module, torch.Tensor), "name=None 일 때는 module에 텐서를 전달해야 합니다."
            t = module
            # 새 pruning method 인스턴스 생성 (훅/버퍼 미사용)
            method = cls(*args, **kwargs)
            method._tensor_name = None

            # 중요도 점수가 없으면 입력 텐서 자체 사용
            if importance_scores is not None:
                assert importance_scores.shape == t.shape, \
                    f"importance_scores shape mismatch: {importance_scores.shape} vs {t.shape}"
            else:
                importance_scores = t
            ...
            default_mask = torch.ones_like(t)
             # 4D 마스크 계산
            mask_full = method.compute_mask(importance_scores, default_mask=default_mask, module_name=module_name)
            mask_full = mask_full.to(dtype=t.dtype, device=t.device)

            # 채널 단위 요약(예전 로직과 동일한 형태: 1D)
            # 과거에 L1-norm을 썼다면 아래 라인 유지
            mask_ch = torch.norm(mask_full, p=1, dim=(1, 2, 3))
            # (binary mask면 sum과 동일)
            # mask_ch = mask_full.view(mask_full.size(0), -1).sum(dim=1)
            pruned = t * mask_full
            # 텐서 경로는 훅/버퍼/파라미터 재배치 없이 곧바로 결과 반환
            return pruned, mask_full, mask_ch

        method = _get_composite_method(cls, module, name, module_name, *args, **kwargs)
        # at this point we have no forward_pre_hooks but we could have an
        # active reparametrization of the tensor if another pruning method
        # had been applied (in which case `method` would be a PruningContainer
        # and not a simple pruning method).
        ...
        try:
            # get the final mask, computed according to the specific method
            mask = method.compute_mask(importance_scores, default_mask=default_mask, module_name= module_name)
            # reparameterize by saving mask to `module[name + '_mask']`...
            module.register_buffer(name + "_mask", mask)
            # ... and the new pruned tensor to `module[name]`
            setattr(module, name, method.apply_mask(module))
    ...
    def prune(self, t, default_mask=None, importance_scores=None):
        ...
        if importance_scores is not None:
            ...
            importance_scores = t
        default_mask = default_mask if default_mask is not None else torch.ones_like(t)
        return t * self.compute_mask(importance_scores, default_mask=default_mask, module_name= module_name)
...
class L1Unstructured(BasePruningMethod):
    ...
    @classmethod
    def apply(cls, module, name, module_name, amount, importance_scores=None):
        r"""Add pruning on the fly and reparametrization of a tensor.
        ...
        """
        return super().apply(
            module, name, module_name=module_name, amount=amount, importance_scores=importance_scores, 
        )
...
def l1_unstructured(module, name, module_name, amount, importance_scores=None):
    ...
    L1Unstructured.apply(
        module, name, amount=amount, importance_scores=importance_scores, module_name= module_name
    )
    return module
